...
- ✅ **Автосохранение прогресса игры** (№2 бэклога): буквы + таймер + остаток подсказок пишутся в `localStorage` (`kraswords_progress_<id>`), при возврате на кроссворд молча восстанавливаются; «сброс» и решение чистят сейв; полностью решённый сейв не восстанавливается. Проверено в браузере: сейв → reload → буква/таймер 0:42/💡 1 на месте; сброс чистит.
- 🎨 **Стиль hero-картинок** — Асил забраковал первые генерации («не в тематике»), канон = rickmorty-wide.jpg. Новая формула: у каждой франшизы свой культовый символ, собранный из кроссвордных клеток (маска Дали из плиток, глаз Блю-Лока из клеток и т.п.) + один цвет свечения. Два теста понравились структурно, ждём выбор варианта Титанов (1/2/3, preview-titan-v*.jpg в bucket). После выбора — перегенерация всех тем.
- ⚠️ Урок: не гонять `npm run build` при живом `next dev` — затирает `.next`, dev начинает отдавать stale-чанки (порядок: сначала build, потом dev, или рестарт dev).
